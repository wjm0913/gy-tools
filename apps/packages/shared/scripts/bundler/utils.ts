...
export type TaksName = 'electron' | 'win-main'
...
export const build = async(config: UserConfig, onUpdated: () => void): Promise<{ status: boolean, reload: () => void }> => {
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    return viteBuild({ ...config, configFile: false }).then((output) => {
      // output
      // console.log(output)
 ...
 * @returns is success
 */
export const runBuildWorker = async(taskName: TaksName, onUpdated: () => void) => new Promise<{ status: boolean, reload: () => void }>((resolve) => {
